...
export function DynamicIcon({ name, active }: { name: string; active: boolean }) {
  ...
  const fillable = ["Heart", "Star", "Bookmark", "ThumbsUp", "ThumbsDown"].includes(name);
  return (
    <Icon
      size={16}
...
export function highlightContext(win: ContextWindow): React.ReactNode {
  ...
  const sorted = [...matches].sort((a, b) => a.offset_in_context - b.offset_in_context);
  const parts: React.ReactNode[] = [];
  let pos = 0;
  sorted.forEach((m, i) => {
    if (m.offset_in_context > pos)
      parts.push(<span key={`t${i}`}>{context.slice(pos, m.offset_in_context)}</span>);
    parts.push(
      <strong key={`m${i}`}>
        {context.slice(m.offset_in_context, m.offset_in_context + m.match_length)}
      </strong>
    );
    pos = m.offset_in_context + m.match_length;
  });
  if (pos < context.length) parts.push(<span key="tail">{context.slice(pos)}</span>);
  return <>{parts}</>;
}
        ...
        <div>
          <div className="min-w-0">
            ...
            {matchedConditions && matchedConditions.length > 0 && (
              <div className="my-3">
              <p className="text-xs text-muted-foreground mb-1">Matched search conditions</p>
              <div className="flex flex-wrap gap-1">
                {matchedConditions.map((c) => (
                  <Badge key={c.id} variant="secondary" className="font-mono text-xs font-normal">
                    {c.regex}
                    {c.not_regex_nearby && (
                      <span className="text-muted-foreground ml-1">· not near {c.not_regex_nearby}</span>
                    )}
                  </Badge>
                ))}
              </div>
